...
path_output = "Extracted_Features/references_clip_features.npy"
...
def Transform(image, apply_transform, transform_params):
    if apply_transform == "jpeg": #params: QF in [50,100]
        ...
        img_np = np.asarray(image.resize((int(transform_params["scale"]*image.size[0]), int(transform_params["scale"]*image.size[1]))).rotate(transform_params["angle"]))
        image.close()
        ...
        if noise_np.shape[0] > img_np.shape[0]:
            noise_np = noise_np[:img_np.shape[0]-noise_np.shape[0],:]
        if noise_np.shape[1] > img_np.shape[1]:
            noise_np = noise_np[:,:img_np.shape[1]-noise_np.shape[1]]
        if noise_np.shape[0] < img_np.shape[0]:
            noise_np = np.concatenate((noise_np, np.zeros((img_np.shape[0]-noise_np.shape[0], noise_np.shape[1]))), axis=0)
        if noise_np.shape[1] < img_np.shape[1]:
            noise_np = np.concatenate((noise_np, np.zeros((img_np.shape[0], img_np.shape[1]-noise_np.shape[1]))), axis=1)
...
#numpyfy and save list of lists of reference feature vectors
